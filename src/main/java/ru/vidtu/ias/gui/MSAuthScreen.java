...
public class MSAuthScreen extends GuiScreen {
	...
	private void auth(String query) {
		try {
			...
			if (query.equals("error=access_denied&error_description=The user has denied access to the scope requested by the client application."))
				throw new AuthException(new TextComponentTranslation("ias.msauth.error.revoked"));
	...
	public void error(Throwable t) {
		mc.addScheduledTask(() -> {
			if (t instanceof AuthException) {
				error = fontRenderer.listFormattedStringToWidth(((AuthException)t).getText().getFormattedText(), width - 20);
			} else {
				error = fontRenderer.listFormattedStringToWidth(I18n.format("ias.auth.unknown", t.toString()), width - 20);
			}
		});
	}
	...
	@Override
	public void initGui() {
		addButton(new GuiButton(0, this.width / 2 - 75, this.height - 28, 150, 20, I18n.format("gui.cancel")));
	}
	...
	public void drawScreen(int mouseX, int mouseY, float delta) {
		...
		drawCenteredString(fontRenderer, I18n.format("ias.msauth.title"), this.width / 2, 7, -1);
		if (error != null) {
			for (int i = 0; i < error.size(); i++) {
				drawCenteredString(fontRenderer, error.get(i), this.width / 2, height / 2 - 5 + i * 10 - error.size() * 5, 0xFFFF0000);
				if (i > 6) break; //Exceptions can be very large.
			}
		} else {
			drawCenteredString(fontRenderer, state, width / 2, height / 2 - 10, -1);
			drawCenteredString(fontRenderer, symbols[tick % symbols.length], width / 2, height / 2, 0xFFFF9900);
		}
