...
public class MSAuthScreen extends Screen {
	...
	public MSAuthScreen(Screen prev, Consumer<Account> handler) {
		...
		new Thread(() -> {
			try {
	        	...
	        	srv.start();
	        	Util.getOSType().openURI("https://login.live.com/oauth20_authorize.srf" +
	...
	public void error(Throwable t) {
		minecraft.execute(() -> {
			if (t instanceof AuthException) {
				error = font.listFormattedStringToWidth(((AuthException)t).getText().getFormattedText(), width - 20);
			} else {
				error = font.listFormattedStringToWidth(I18n.format("ias.auth.unknown", t.toString()), width - 20);
			}
		});
	}
	...
	@Override
	public void init() {
		addButton(new Button(this.width / 2 - 75, this.height - 28, 150, 20, I18n.format("gui.cancel"), btn -> minecraft.displayGuiScreen(prev)));
	}
	...
	public void render(int mouseX, int mouseY, float delta) {
		...
		if (error != null) {
			for (int i = 0; i < error.size(); i++) {
				font.drawStringWithShadow(error.get(i), this.width / 2 - font.getStringWidth(error.get(i)) / 2, height / 2 - 5 + i * 10 - error.size() * 5, 0xFFFF0000);
