...
public class MSAuthScreen extends Screen {
	...
	private int tick;
	private String state = I18n.translate("ias.msauth.checkbrowser");
	...
	private void auth(String query) {
		try {
			state = I18n.translate("ias.msauth.progress");
	...
	public void error(Throwable t) {
		minecraft.execute(() -> {
			if (t instanceof AuthException) {
				error = font.wrapStringToWidthAsList(((AuthException)t).getText().asFormattedString(), width - 20);
			} else {
				error = font.wrapStringToWidthAsList(I18n.translate("ias.auth.unknown", t.toString()), width - 20);
			}
		});
	}
	...
	@Override
	public void init() {
		addButton(new ButtonWidget(this.width / 2 - 75, this.height - 28, 150, 20, I18n.translate("gui.cancel"), btn -> minecraft.openScreen(prev)));
	}
