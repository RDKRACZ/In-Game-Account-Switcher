...
public class IASConfigScreen extends GuiScreen {
	...
	@Override
	public void initGui() {
		addButton(caseS = new GuiCheckBox(1, width / 2 - fontRenderer.getStringWidth(I18n.format("ias.cfg.casesensitive")) / 2 - 8, 40, I18n.format("ias.cfg.casesensitive"), Config.caseSensitiveSearch));
		addButton(mpscreen = new GuiCheckBox(2, width / 2 - fontRenderer.getStringWidth(I18n.format("ias.cfg.mpscreen")) / 2 - 8, 60, I18n.format("ias.cfg.mpscreen"), Config.showOnMPScreen));
		addButton(titlescreen = new GuiCheckBox(3, width / 2 - fontRenderer.getStringWidth(I18n.format("ias.cfg.titlescreen")) / 2 - 8, 80, I18n.format("ias.cfg.titlescreen"), Config.showOnTitleScreen));
		textX = new SuggestorTextField(4, fontRenderer, width / 2 - 100, 110, 98, 20, "X");
		textY = new SuggestorTextField(5, fontRenderer, width / 2 + 2, 110, 98, 20, "Y");
		btnX = new SuggestorTextField(6, fontRenderer, width / 2 - 100, 152, 98, 20, "X");
		btnY = new SuggestorTextField(7, fontRenderer, width / 2 + 2, 152, 98, 20, "Y");
	...
	public void drawScreen(int mx, int my, float delta) {
		...
		drawCenteredString(fontRenderer, "config/ias.json", width / 2, 10, -1);
		drawCenteredString(fontRenderer, I18n.format("ias.cfg.textpos"), width / 2, 100, -1);
		if (titlescreen.isChecked()) drawCenteredString(fontRenderer, I18n.format("ias.cfg.btnpos"), width / 2, 142, -1);
		textX.drawTextBox();
		textY.drawTextBox();
		btnX.drawTextBox();
		btnY.drawTextBox();
