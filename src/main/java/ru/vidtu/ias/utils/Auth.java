    ...
    public static MojangAccount authMojang(String name, String pwd) throws AuthException, IOException {
		...
		if (r.response() < 200 || r.response() >= 300) {
			...
			try {
				...
				throw new AuthException(new TranslatableComponent("ias.mojauth.unknown", reqerr), reqerr);
			} 
			...
			throw new AuthException(new TranslatableComponent("ias.mojauth.unknown", jo.toString()));
		}
		...
		if (!respClientToken.equals(clientToken))
			throw new AuthException(new TranslatableComponent("ias.mojauth.unknown",
