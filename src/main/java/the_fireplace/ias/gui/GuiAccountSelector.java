...
public class GuiAccountSelector extends Screen {
	...
	protected void init() {
		...
		addChild(accountsgui);
		addButton(reloadskins = new ButtonWidget(2, 2, 120, 20, new TranslatableText("ias.reloadskins"), btn -> reloadSkins()));
		addButton(new ButtonWidget(this.width / 2 + 4 + 40, this.height - 52, 120, 20, new TranslatableText("ias.addaccount"), btn -> add()));
		addButton(login = new ButtonWidget(this.width / 2 - 154 - 10, this.height - 52, 120, 20, new TranslatableText("ias.login"), btn -> accountsgui.login()));
		addButton(edit = new ButtonWidget(this.width / 2 - 40, this.height - 52, 80, 20, new TranslatableText("ias.edit"), btn -> accountsgui.edit()));
		addButton(loginoffline = new ButtonWidget(this.width / 2 - 154 - 10, this.height - 28, 110, 20, new TranslatableText("ias.login").append(" ").append(new TranslatableText("ias.offline")), btn -> accountsgui.loginOffline()));
		addButton(new ButtonWidget(this.width / 2 + 4 + 50, this.height - 28, 110, 20, new TranslatableText("gui.cancel"), btn -> client.openScreen(prev)));
		addButton(delete = new ButtonWidget(this.width / 2 - 50, this.height - 28, 100, 20, new TranslatableText("ias.delete"), btn -> accountsgui.delete()));
	...
	public void render(MatrixStack ms, int mx, int my, float delta) {
		...
		if (accountsgui.getSelected() != null) {
			...
			client.getTextureManager().bindTexture(accountsgui.getSelected().model(false));
	 ...
	 */
	private void add() {
		client.openScreen(new AbstractAccountGui(this, new TranslatableText("ias.addaccount"), acc -> {
	...
	public class AccountList extends AlwaysSelectedEntryListWidget<AccountEntry> {
		...
		public void edit() {
			...
			client.openScreen(new AbstractAccountGui(GuiAccountSelector.this, new TranslatableText("ias.editaccount"), acc -> {
		...
		public void delete() {
			...
			client.openScreen(new ConfirmScreen(b -> {
				...
				client.openScreen(GuiAccountSelector.this);
	...
	public class AccountEntry extends AlwaysSelectedEntryListWidget.Entry<AccountEntry> {
		...
		public void render(MatrixStack ms, int i, int y, int x, int w, int h, int mx, int my, boolean hover, float delta) {
			...
			client.getTextureManager().bindTexture(face(false));
			Screen.drawTexture(ms, x, y + 1, 0, 0, 8, 8, 8, 8);
			if (accountsgui.getSelected() == this) {
				client.getTextureManager().bindTexture(new Identifier("textures/gui/server_selection.png"));
