...
public class GuiAccountSelector extends Screen {
	...
	protected void init() {
		...
		addButton(reloadskins = new Button(2, 2, 120, 20, I18n.format("ias.reloadskins"), btn -> reloadSkins()));
		addButton(new Button(this.width / 2 + 4 + 40, this.height - 52, 120, 20, I18n.format("ias.addaccount"), btn -> add()));
		addButton(login = new Button(this.width / 2 - 154 - 10, this.height - 52, 120, 20, I18n.format("ias.login"), btn -> accountsgui.login()));
		addButton(edit = new Button(this.width / 2 - 40, this.height - 52, 80, 20, I18n.format("ias.edit"), btn -> accountsgui.edit()));
		addButton(loginoffline = new Button(this.width / 2 - 154 - 10, this.height - 28, 110, 20, new TranslationTextComponent("ias.login").appendText(" ").appendSibling(new TranslationTextComponent("ias.offline")).getFormattedText(), btn -> accountsgui.loginOffline()));
		addButton(new Button(this.width / 2 + 4 + 50, this.height - 28, 110, 20, I18n.format("gui.cancel"), btn -> minecraft.displayGuiScreen(prev)));
		addButton(delete = new Button(this.width / 2 - 50, this.height - 28, 100, 20, I18n.format("ias.delete"), btn -> accountsgui.delete()));
		addButton(search = new TextFieldWidget(this.font, this.width / 2 - 80, 14, 160, 16, I18n.format("ias.search")));
	    updateButtons();
	    search.setSuggestion(I18n.format("ias.search"));
	...
	public void tick() {
		...
		if (!prevQuery.equals(search.getText())) {
			...
			search.setSuggestion(search.getText().isEmpty()?I18n.format("ias.search"):"");
		}
	}
	...
	public void render(int mx, int my, float delta) {
		...
		drawCenteredString(font, this.title.getFormattedText(), this.width / 2, 4, -1);
		if (error != null) {
		...
		if (accountsgui.getSelected() != null) {
			...
			if (acc.online()) drawString(font, I18n.format("ias.premium"), width - 8 - 61, height / 2 - 64 - 13, 6618980);
			else drawString(font, I18n.format("ias.notpremium"), width - 8 - 61, height / 2 - 64 - 13, 16737380);
			drawString(font, I18n.format("ias.timesused"), width - 8 - 61, height / 2 - 64 - 15 + 12, -1);
			drawString(font, String.valueOf(acc.uses()), width - 8 - 61, height / 2 - 64 - 15 + 21, -1);
			if (acc.uses() > 0) {
				drawString(font, I18n.format("ias.lastused"), width - 8 - 61, height / 2 - 64 - 15 + 30, -1);
	 ...
	 */
	private void add() {
		minecraft.displayGuiScreen(new AbstractAccountGui(this, new TranslationTextComponent("ias.addaccount"), acc -> {
	...
	public class AccountList extends ExtendedList<AccountEntry> {
		...
		public void loginOffline() {
			...
			Account acc = getSelected().account;
			acc.use();
			minecraft.session = new Session(acc.alias(), UUIDTypeAdapter.fromUUID(new UUID(0, 0)), "0", "legacy");
		}
		
		public void edit() {
			if (empty() || !getSelected().account.editable()) return;
			minecraft.displayGuiScreen(new AbstractAccountGui(GuiAccountSelector.this, new TranslationTextComponent("ias.editaccount"), acc -> {
		...
		public void delete() {
			...
			minecraft.displayGuiScreen(new ConfirmScreen(b -> {
				...
				minecraft.displayGuiScreen(GuiAccountSelector.this);
	...
	public class AccountEntry extends ExtendedList.AbstractListEntry<AccountEntry> {
		...
		public void render(int i, int y, int x, int w, int h, int mx, int my, boolean hover, float delta) {
			...
			if (accountsgui.getSelected() == this) {
				minecraft.getTextureManager().bindTexture(new ResourceLocation("textures/gui/server_selection.png"));
		...
		public ResourceLocation model(boolean forceReload) {
			if (forceReload) {
				minecraft.getTextureManager().deleteTexture(modelTexture);
				modelTexture = null;
			}
			if (modelTexture == null) {
				File model = new File(new File(minecraft.gameDir, "cachedImages/models"), account.alias() + ".png");
				File face = new File(new File(minecraft.gameDir, "cachedImages/faces"), account.alias() + ".png");
				...
				try (FileInputStream fis = new FileInputStream(model); NativeImage ni = NativeImage.read(fis)) {
					...
					modelTexture = minecraft.getTextureManager().getDynamicTextureLocation("iasmodel_" + account.alias().hashCode(), nibt);
				} catch (Throwable t) {
		...
		public ResourceLocation face(boolean forceReload) {
			if (forceReload) {
				minecraft.getTextureManager().deleteTexture(faceTexture);
				faceTexture = null;
			}
			if (faceTexture == null) {
				File model = new File(new File(minecraft.gameDir, "cachedImages/models"), account.alias() + ".png");
				...
				try (FileInputStream fis = new FileInputStream(face); NativeImage ni = NativeImage.read(fis)) {
					DynamicTexture nibt = new DynamicTexture(ni);
					faceTexture = minecraft.getTextureManager().getDynamicTextureLocation("iasface_" + account.alias().hashCode(), nibt);
				} catch (Throwable t) {
