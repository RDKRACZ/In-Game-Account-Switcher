...
public class AbstractAccountGui extends GuiScreen {
	...
	@Override
	public void initGui() {
		complete = addButton(new GuiButton(0, this.width / 2 - 152, this.height - 28, 150, 20, this.title));
		addButton(new GuiButton(1, this.width / 2 + 2, this.height - 28, 150, 20, I18n.format("gui.cancel")));
		username = new GuiTextField(2, this.fontRenderer, this.width / 2 - 100, 60, 200, 20);
		username.setMaxStringLength(512);
		password = new GuiPasswordField(3, this.fontRenderer, this.width / 2 - 100, 90, 200, 20);
		password.setMaxStringLength(512);
		complete.enabled = false;
		addButton(new GuiButton(4, this.width / 2 - 50, this.height / 3 * 2, 100, 20, I18n.format("ias.msauth.btn")));
	}
	...
	public void drawScreen(int mx, int my, float delta) {
		...
		drawCenteredString(fontRenderer, this.title, this.width / 2, 7, -1);
		drawCenteredString(fontRenderer, I18n.format("ias.username"), this.width / 2 - 130, 66, -1);
		drawCenteredString(fontRenderer, I18n.format("ias.password"), this.width / 2 - 130, 96, -1);
		if (error != null) {
			for (int i = 0; i < error.size(); i++) {
				drawCenteredString(fontRenderer, error.get(i), this.width / 2, 114 + i * 10, 0xFFFF0000);
	...
	public void end() {
		if (password.getText().isEmpty()) {
			...
			if (Config.accounts.stream().anyMatch(acc -> acc.alias().equalsIgnoreCase(name))) {
				error = fontRenderer.listFormattedStringToWidth(I18n.format("ias.auth.alreadyexists"), width - 10);
				return;
			}
			logging = true;
			new Thread(() -> {
			...
			new Thread(() -> {
				try {
					...
					if (Config.accounts.stream().anyMatch(acc -> acc.alias().equalsIgnoreCase(name)))
						throw new AuthException(new TextComponentTranslation("ias.auth.alreadyexists"));
					mc.addScheduledTask(() -> {
						if (mc.currentScreen == this) {
							handler.accept(ma);
							mc.displayGuiScreen(prev);
					...
					mc.addScheduledTask(() -> error = fontRenderer.listFormattedStringToWidth(ae.getText().getFormattedText(), width - 10));
				} catch (Throwable t) {
					IAS.LOG.warn("Unable to add account (unexpected exception)", t);
					mc.addScheduledTask(() -> error = fontRenderer.listFormattedStringToWidth(I18n.format("ias.auth.unknown", t.getLocalizedMessage()), width - 10));
				}
				logging = false;
