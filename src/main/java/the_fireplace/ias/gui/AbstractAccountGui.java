...
public class AbstractAccountGui extends Screen {
	...
	@Override
	public void init() {
		complete = addButton(new ButtonWidget(this.width / 2 - 152, this.height - 28, 150, 20, this.title, btn -> end()));
		addButton(new ButtonWidget(this.width / 2 + 2, this.height - 28, 150, 20, new TranslatableText("gui.cancel"), btn -> client.openScreen(prev)));
		username = addButton(new TextFieldWidget(this.textRenderer, this.width / 2 - 100, 60, 200, 20, new TranslatableText("ias.username")));
		username.setMaxLength(512);
		password = addButton(new GuiPasswordField(this.textRenderer, this.width / 2 - 100, 90, 200, 20, new TranslatableText("ias.password")));
		password.setMaxLength(512);
		complete.active = false;
		addButton(new ButtonWidget(this.width / 2 - 50, this.height / 3 * 2, 100, 20, new TranslatableText("ias.msauth.btn"), btn -> client.openScreen(new MSAuthScreen(prev, handler))));
	}
